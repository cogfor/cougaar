 ...
 * looking for new MicroAgents with MotionDetector capabilities, and allocating tasks to them.
 * It expects the MicroAgents' capabilities to have a colon-separated format like:
 * <pre>
 *              MotionDetector:ReportState0:0.0:1.0:0.0:0.0
		MotionDetector:ReportState1:1.0:1.0:0.0:0.0
		MotionDetector:ReportState2:2.0:1.0:0.0:0.0
 * </pre>
 * This example shows 3 motion detectors.  The fields on each line are:
 * <nl><li>The constant "MotionDetector" all else is ignored
 ...
 * <li>The sensor's bearing</nl>
 */
public class MotionDetectorPlugin extends ComponentPlugin {
  ...
  private void makeTasks(MicroAgent micro) {
    ...
    while (toker.hasMoreTokens()) {
        ...
        pp.setPreposition("MotionDetector");
        ...
        pp.setIndirectObject("MotionDetector-"+mapg.getName()+"-"+detectorNumber++);
        t.addPrepositionalPhrase(pp);
        if (loggingService.isDebugEnabled())
